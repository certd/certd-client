...
- 客户端版本统一由 `internal/version.Version` 管理，必须符合 Node.js SemVer；发布标签使用 `vX.Y.Z`，版本号按 Conventional Commits 自动递增，破坏性变更为 major、`feat` 为 minor、`fix` 与 `perf` 为 patch。`scripts/release.ps1` 必须在修改版本、提交、打标签和推送之前执行本地 `go test ./...` 与 `go vet ./...`，任一失败即取消发布；首次发布尚无 `v*` 标签时，CHANGELOG 必须包含完整 Git 历史，后续发布仅记录上一版本标签后的提交。
