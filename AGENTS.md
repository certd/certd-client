...
- GitHub 发布流程需先完成跨平台构建并创建 GitHub Release；Release 正文必须提取 `CHANGELOG.md` 中当前版本段的提交条目，不能使用 GitHub 自动生成的 Full Changelog。GitHub push 通过 `atomgit.com` 同步代码，GitHub Release 发布成功后通过 `https://api.atomgit.com/api/v5` 创建同版本 Release。附件先调用 `releases/{tag}/upload_url` 获取签名地址，再按返回的请求头使用 `PUT` 上传。所有 AtomGit 令牌只能通过 GitHub Secret 传入，禁止写入仓库。
