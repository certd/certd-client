...
请确认 IIS 站点已经创建 HTTPS 绑定，并且绑定中配置了域名主机名（Host name），例如 `example.com` 或 `www.example.com`。只有 IP、通配符 `*` 或空主机名的绑定无法识别出证书申请域名；同一个 IIS 站点的多个域名绑定会合并为一次证书申请。

### 证书一直显示“正在申请中”

当证书正在申请中时，客户端会每 10 秒向 Certd 重新查询。请检查 Certd 域名管理中是否已配置域名校验方式、自动化流水线是否可用，以及接口设置中的最长等待时长是否足够。
